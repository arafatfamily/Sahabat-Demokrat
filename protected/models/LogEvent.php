...
class LogEvent extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('ses_id, users_id, act_type, ip_address', 'required'),
			array('ses_id', 'numerical', 'integerOnly'=>true),
			array('users_id', 'length', 'max'=>10),
			array('act_type', 'length', 'max'=>6),
			array('date_time, act_info', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('log_id, ses_id, date_time, users_id, act_info, act_type, ip_address', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'ses' => array(self::BELONGS_TO, 'EventSesi', 'ses_id'),
